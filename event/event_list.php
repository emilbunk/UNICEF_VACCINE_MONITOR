...
<table class="catlist" style="">
  ...
  <tr class="d<?php echo ($i & 1); ?>" >
    ...
    if ($item['eventtype']==7) echo "out of range";
    ?></td>
    ...
    if ($item['action']==5) echo "send SMS"; 
    ?></td>
  ...
  <div style=" background-color:#eee; margin-bottom:10px; border: 1px solid #ddd">
    <div style="padding:10px;  border-top: 1px solid #fff; ">
      ...
      <select id="action" name="action" style="width:100px; margin:0px;">
          ...
          <option value="5" >send SMS</option>
      </select>
      ...
      <span id="not-sms" style="display:none" style="font-weight:bold;">to
          <input name="setphonenumber" type="text" style="width:180px; margin:0px;" />
      </span>
...
<script type="application/javascript">
  ...
  $("#action").click(function() {
    if ($(this).val() == 0) { $("#not-email").show(); $("#not-sms").hide(); $("#not-curl").hide(); $("#not-feed").hide(); $("#not-value").hide(); $("#not-message").show(); $("#not-prowl").hide();}
    if ($(this).val() == 1) { $("#not-email").hide(); $("#not-sms").hide(); $("#not-curl").hide(); $("#not-feed").show(); $("#not-value").show(); $("#not-message").hide(); $("#not-prowl").hide();}
    if ($(this).val() == 2) { $("#not-email").hide(); $("#not-sms").hide(); $("#not-curl").show(); $("#not-feed").hide(); $("#not-value").hide(); $("#not-message").hide(); $("#not-prowl").hide();}
    if ($(this).val() == 3) { $("#not-email").hide(); $("#not-sms").hide(); $("#not-curl").hide(); $("#not-feed").hide(); $("#not-value").hide(); $("#not-message").show(); $("#not-prowl").hide();}
    if ($(this).val() == 4) { $("#not-email").hide(); $("#not-sms").hide(); $("#not-curl").hide(); $("#not-feed").hide(); $("#not-value").hide(); $("#not-message").show(); $("#not-prowl").show();}
    if ($(this).val() == 5) { $("#not-email").hide(); $("#not-sms").show(); $("#not-curl").hide(); $("#not-feed").hide(); $("#not-value").hide(); $("#not-message").show(); $("#not-prowl").hide();}
  });
</script>
