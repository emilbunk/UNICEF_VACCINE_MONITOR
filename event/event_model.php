...
function event_add($userid,$eventfeed,$eventtype,$eventvalue,$action,$setfeed,$setemail,$setvalue,$callcurl,$message,$mutetime,$priority,$delaytime,$setphonenumber)
{
  if($eventtype == 7) {$eventfeed = 0;}
  db_query("INSERT INTO event (`userid`,`eventfeed`, `eventtype`, `eventvalue`, `action`, `setfeed`, `setemail`, `setvalue`, `lasttime`, `callcurl`, `mutetime`, `priority`, `delaytime`,`setphonenumber`, `message`) VALUES ('$userid','$eventfeed','$eventtype','$eventvalue','$action','$setfeed','$setemail','$setvalue','0','$callcurl','$mutetime','$priority','$delaytime','$setphonenumber','$message')");
}
...
function check_feed_event($feedid,$updatetime,$feedtime,$value,$row=NULL) {
	...
	$feed = db_query("SELECT * FROM feeds WHERE id = $feedid");
	$feed = db_fetch_array($feed);
    $result = db_query("SELECT * FROM event WHERE eventfeed = $feedid OR eventfeed = '0'");
    ...
    while ($row = db_fetch_array($result)) {
        ...
        switch($row['eventtype']) {
            ...
            case 7:
            	// UNICEF
            	if (strcmp("fridge", $feed['tag']) and ($value > 7 or $value < 3)) {
                    $sendAlert = 1;
                }
            	break;
        }
